...
def rank_weights(dirNo):
	...
	documents = []
	for file in os.listdir('./Combined_candidate_3'+str(dirNo)):
		with open(os.path.join('./Combined_modified_3'+str(dirNo), file), 'r') as f2:
			text = f2.read()
	...
	for file in os.listdir('./Combined_candidate_3'+str(dirNo)):
		...
		# 	document = text
		# f2.close()
		with open(os.path.join('./Combined_candidate_3'+str(dirNo),file), 'r') as f2:
			text = f2.read()
			text = text.replace('{','')
			text = text.replace('}','')
			text = text.replace('\'','')
