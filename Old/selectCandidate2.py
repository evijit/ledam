...
def select(dirNo):
	...
	for file in os.listdir('./OldMethod'+str(dirNo)):
		with open(os.path.join('./OldMethod'+str(dirNo), file), 'r') as f:
			text = f.read()
		f.close()
		with open(os.path.join('./OldCandidate', file), 'r') as f:
			cand = f.read()
			cand = cand.replace('{','')
			cand = cand.replace('}','')
			cand = cand.replace('\'','')
		...
		# print(len(candidate))
		with open(os.path.join('./Phrase_weights_new', file), 'r') as f:
			weights = f.read()
			wts = weights.split(',')
		f.close()
		...
		for i1, c1 in enumerate(candidate):
			...
			for i2,c2 in enumerate(candidate):
				...
				try:
					...
					pos2 = text.index(c2)
					# print(pos1, pos2)
					if pos1-pos2<=5 or pos2-pos1<=5:
						wt = spatial.distance.cosine(candidate_vec[i1], candidate_vec[i2])
						if np.isnan(wt[0]):
							g.add_edge(i1, i2, weight=0)
						else:
							g.add_edge(i1, i2, weight=wt[0])
				except Exception as e:
					continue
		print("Graph created")
		pr = g.pagerank(niter=50)
		print(pr)
	...
	# 	final_ranks.append(rank_score)
		...
		with open(os.path.join('./Results_new2', file), 'w') as re:
		...
		# left_r = len(set(phrase_list)-set(extracted_phrases_rec))
		# left= len(set(phrase_list)- set(extracted_phrases))
